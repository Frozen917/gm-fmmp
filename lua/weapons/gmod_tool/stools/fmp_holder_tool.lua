...
function TOOL:LeftClick(trace)
	if SERVER then
		...
		local spawnFunction = Util.GetStoredMember("f2mp_base_holder", "SpawnFunction")
		if spawnFunction then
			local entity = spawnFunction(Util.GetStoredEntity(className), self:GetOwner(), trace, self:GetClientInfo("freeze") == "1")
			entity:Setup(typ)
		else
