local function load()
	if SERVER then
		RunConsoleCommand("sv_allowcslua", "1")
		util.AddNetworkString("UpdateLocalEnvironment")
		AddCSLuaFile("autorun/loader.lua")
		AddCSLuaFile("../../lua/headers/lol.lua")
		AddCSLuaFile("../../lua/headers/container_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/container_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/filter_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/filter_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/fmp_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/fmp_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/generator_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/generator_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/holdable_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/holdable_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/holder_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/holder_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/pluggable_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/pluggable_entity/cl_init.lua")
		AddCSLuaFile("../../lua/headers/rd_entity/shared.lua")
		AddCSLuaFile("../../lua/headers/rd_entity/cl_init.lua")
		AddCSLuaFile("core/shared/environment.lua")
		AddCSLuaFile("core/shared/slot.lua")
		AddCSLuaFile("core/shared/core.lua")
		AddCSLuaFile("core/client/core.lua")
		AddCSLuaFile("core/shared/plugins.lua")
		AddCSLuaFile("core/shared/space.lua")
		AddCSLuaFile("core/shared/util.lua")
		AddCSLuaFile("core/shared/plug.lua")
		AddCSLuaFile("core/shared/pipe.lua")
		include("core/shared/util.lua")
		include("core/shared/environment.lua")
		include("core/server/universe.lua")
		include("core/shared/plug.lua")
		include("core/shared/slot.lua")
		include("core/shared/pipe.lua")
		include("core/server/core.lua")
		include("core/shared/core.lua")
		include("core/server/initializer.lua")
		include("core/shared/plugins.lua")
		include("core/shared/space.lua")
		include("core/server/resources.lua")
	elseif CLIENT then
		include("core/shared/util.lua")
		include("core/shared/environment.lua")
		include("core/shared/plug.lua")
		include("core/shared/slot.lua")
		include("core/shared/pipe.lua")
		include("core/client/core.lua")
		include("core/shared/core.lua")
		include("core/shared/plugins.lua")
		include("core/shared/environment.lua")
		include("core/shared/space.lua")
		end
	print("Gamemode loaded !")
end
load()
concommand.Add("gmreload", load)